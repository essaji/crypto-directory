...
export default function HomePage() {
  const {
    isLoading,
    data: exchanges
  } = useQuery("exchangeData", () => fetchExchanges({per_page: 10}), {staleTime: Infinity})
  ...
  return (
    <div className="home-page">
      ...
      <div className="home-page__exchanges">
        {exchanges?.data.map((exchange: ExchangeInterface) => <ExchangeCard {...exchange} key={exchange.id} />)}
      </div>
    </div>
  )
}